...
void processLidar()
{
  if (IS_OK(lidar.waitPoint()))
  {
    ...
    // double angle = (lidar.getCurrentPoint().angle + robot.current_pose.bearing) < 360 ? (lidar.getCurrentPoint().angle + robot.current_pose.bearing) : 360 - (lidar.getCurrentPoint().angle + robot.current_pose.bearing);
    double angle = (lidar.getCurrentPoint().angle + robot.current_pose.bearing);
